...
test('follow-up draft save never sends LINE and requires explicit confirmation', () => {
  ...
  const end = worker.indexOf("if (pathname === '/api/admin/followups/drafts')", start);
...
test('draft list is read-only and exposes no LINE recipient identifier', () => {
  ...
  assert.ok(start > 0 && end > start);
  const route = worker.slice(start, end);
  assert.match(route, /WHERE status IN \('draft', 'approved', 'sending', 'sent', 'failed'\)/);
...
test('draft approval records approval but never sends LINE', () => {
  ...
  const end = worker.indexOf("if (pathname === '/api/admin/followups/single-send')", start);
  assert.ok(start > 0 && end > start);
  const route = worker.slice(start, end);
